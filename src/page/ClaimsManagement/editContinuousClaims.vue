...
import { addContinuousClaims, getContinuousClaimsDetail } from "@/mock/api";

export default {
  name: "editContinuousClaims",
  data() {
    return {
      form: {
        id: null,
  ...
  methods: {
    ...
    getDetail() {
      getContinuousClaimsDetail({ id: this.form.id }).then(res => {
        const { id, name, username, phone, address, lnglat } = res.Data;
        this.form = {
          id,
          name,
          username,

...
<script>
  ...
  import {ReadPolicyGroup,GetAllStaff,AddStaff,UpdStaff,DelStaff} from "@/request/api"
  export default {
    ...
    methods:{
      ...
      oks:function () {
        ...
        AddStaff({account:that.accounts,role:that.celuezu[that.roles],created_at:new Date()}).then(res=>{
          this.$message(res.Msg);
          that.accountdata=res.Data
          that.addshow=false
          that.mainshow=true
      ...
      del:function () {
        ...
        DelStaff({id:this.currentRow}).then(res=>{
      ...
      getstaff(){
        var that=this
        GetAllStaff({PageIndex:that.PageIndex,PageSize:that.PageSize,key:that.names}).then(res=>{
          var data=res.Data
          window.console.log('员工账号列表',res.Data)

  ...
  <div>
    ...
    <div class="content_box">
      ...
      <el-form ref="form" :model="form" class="search_form">
        <el-form-item>
          <div class="cell_before">员工</div>
          <el-select v-model="form.employeeId" placeholder="请选择员工" clearable filterable>
            <el-option
    ...
    <el-dialog :title="addForm.title" :visible.sync="addForm.isShow">
      <el-form ref="addForm" :model="addForm.data" class="dialog_from_center">
        <el-form-item>
          <div class="cell_before yellow">*员工</div>
          <el-select v-model="addForm.data.employeeId" placeholder="请选择员工" clearable filterable>
            <el-option
              v-for="item in employeeList"
              :key="item.id"
              :label="item.name"
...
import {
  getInsideStaffAccount,
  delInsideStaffAccount,
  getEmployeeList,
  getPermissionListNoPage,
  addInsideStaffAccount
} from "@/mock/api";
export default {
  name: "InsideStaffAccount",
  data() {
    return {
      form: {
        employeeId: null,
  ...
  methods: {
    getList() {
      this.clickRow = {};
      getInsideStaffAccount(this.form).then(res => {
    ...
    getData() {
      getEmployeeList().then(res => {
        this.employeeList = res.Data.jigou;
      });
    ...
    del() {
      delInsideStaffAccount({ id: this.clickRow.id }).then(res => {
    ...
    confirm() {
      if (!this.addForm.data.employeeId) return this.$message("请选择员工");
      ...
      addInsideStaffAccount(this.addForm.data).then(res => {
        this.$message(res.data.Msg);

...
export default {
  ...
  methods: {
    ...
    getList() {
      this.clickRow = {};
      getMyCollegeList(this.form).then(res => {
    ...
    del() {
      delzhishi({ id: this.clickRow.id }).then(res => {
    ...
    changeStatus(n) {
      fabuzhishi({ id: this.clickRow.id, status: n }).then(res => {
        this.$message(res.data.Msg);
        this.getList();
      });
    },

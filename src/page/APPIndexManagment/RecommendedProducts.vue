          ...
          <div class="simulation">
            <ul>
              <li v-for="(item, index) of tableData" :key="index">
                ...
                <div class="desc">
                  <p
                    v-for="(item1, index1) of item.description"
                    :key="index1"
                  >{{item1.name}} {{item1.value}}</p>
                </div>
                <div class="tags">
            ...
            <el-tabs v-model="activeName" type="card">
              <el-tab-pane label="保库险种管理" name="one">
                <div style="text-align: right; margin: 16px 0">
                  <el-button type="warning" @click="addDiaolog(1)" icon="el-icon-plus">新增险种</el-button>
                </div>
                <el-table
                  :data="tableData"
                ...
                >
                  ...
                  <el-table-column prop="description" label="说明" show-overflow-tooltip>
                    <template slot-scope="scope">
                      <span v-for="(item1, index1) in scope.row.description" :key="index1">
                        <span>{{item1.name}}:{{item1.value}}</span>
                        <span v-if="index1<scope.row.description.length-1">|</span>
                      </span>
                    </template>
                  </el-table-column>
                  <el-table-column prop="tags" label="标签">
                    <template slot-scope="scope">
                      <span v-for="(item1, index1) in scope.row.tags" :key="index1">
                        <span>{{item1}}</span>
                        <span v-if="index1<scope.row.tags.length-1">|</span>
                      </span>
                    </template>
                  </el-table-column>
                  <el-table-column prop="order" label="排序号" width="100"></el-table-column>
                  <el-table-column prop="createTime" label="创建时间"></el-table-column>
                  <el-table-column label="操作">
                    <template slot-scope="scope">
                      <router-link
                        :to="{path:'/recommended_products_detail',query:{id:scope.row.id}}"
                      >
                        <el-link :underline="false" type="primary">查看产品详情</el-link>
                      </router-link>
                    </template>
                  </el-table-column>
                </el-table>
  ...
  methods: {
    getSupplierList() {
      getallparameter().then(res => {
        this.addForm.supplier.options = res.Data.gongyingshang.map(i => {
    ...
    getProductList() {
      apiProductList().then(res => {
        this.addForm.name.options = res.Data || [];
      });
    },
    ...
    // 删除
    del() {
      delRecommendProducts({ id: this.clickRow.id }).then(res => {
    ...
    addDiaolog(n) {
      ...
      this.addForm.supplier.disabled = n == 1 ? false : true;
      this.addForm.name.disabled = n == 1 ? false : true;
      this.title = n == 1 ? "添加险种" : "编辑险种";
      this.isShow = true;
    ...
    save() {
      ...
      saveRecommendProducts(data).then(res => {
        this.isShow = false;
        this.$message(res.data.Msg);
      });
    },

      ...
      <div class="body">
        ...
        >
          <template v-if="isShowMask">
            <div
              class="around-mask"
              :style="{left: 0, top: 0, width: centerLeft + 'px', height: wrapStyle.height + 'px'}"
            ></div>
            <div
              class="around-mask"
              :style="{left: centerLeft + 'px', top: 0, width: viewLength + 'px', height: centerTop + 'px'}"
            ></div>
            <div
              class="around-mask"
              :style="{left: centerLeft + viewLength + 'px', top: 0, width: wrapStyle.width - centerLeft - viewLength + 'px', height: wrapStyle.height + 'px'}"
            ></div>
            <div
              class="around-mask"
              :style="{left: centerLeft + 'px', top: centerTop + viewLength + 'px', width: viewLength + 'px', height: wrapStyle.height - centerTop - viewLength + 'px'}"
            ></div>
            <div
              class="center"
            ...
            >
              ...
              <div
                class="line w"
                @mousedown.stop.prevent="wLineMousedown"
                :style="{cursor: 'w-resize', borderLeftWidth: '1px', width: '6px', height: viewLength + 'px'}"
              ></div>
              <div
                class="line n"
                @mousedown.stop.prevent="nLineMousedown"
                :style="{cursor: 'n-resize', borderTopWidth: '1px', width: viewLength + 'px', height: '6px'}"
              ></div>
              <div
                class="line e"
                @mousedown.stop.prevent="eLineMousedown"
                :style="{cursor: 'e-resize', borderRightWidth: '1px', width: '6px', height: viewLength + 'px'}"
              ></div>
              <div
                class="line s"
                @mousedown.stop.prevent="sLineMousedown"
                :style="{cursor: 'n-resize', borderBottomWidth: '1px', width: viewLength + 'px', height: '6px'}"
              ></div>
            </div>
          </template>
          <img
...
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      // 配置长宽
      maxWrap: { width: 300, height: 182.5 },
      preview: { width: 600, height: 365 },
      // 裁剪相关
      centerStart: { x: 0, y: 0 },
      viewLength: 0,
      origin: { x: 0, y: 0 },
      wrapStyle: { width: 300, height: 182.5 },
      moveStart: { x: 0, y: 0 },
      moved: { x: 0, y: 0 },
      isShowMask: false,
      // 文件相关
  ...
  computed: {
    centerLeft() {
      return this.centerStart.x + this.moved.x;
    },
    centerTop() {
      return this.centerStart.y + this.moved.y;
    },
    maxLength() {
      return Math.min(
        this.wrapStyle.width - this.centerLeft,
        this.wrapStyle.height - this.centerTop
      );
    }
  },
  watch: {
    ...
    imgWH: {
      handler({ width, height }) {
        ...
        if (width < height) {
          this.scaleB = this.maxWrap.height / height;
        }
        width *= this.scaleB;
        height *= this.scaleB;
  ...
  methods: {
    ...
    // 重置
    reset() {
      this.wrapStyle = { width: 300, height: 182.5 };
    ...
    wrapMousemove(e) {
      ...
      let length;
      if (
        Math.abs(clientY - y - this.centerTop) >
        Math.abs(clientX - x - this.centerLeft)
      ) {
    ...
    centerMousedown(e) {
      this.moveStart = {
        x: e.clientX - this.moved.x,
        y: e.clientY - this.moved.y
      };
    ...
    centerMousemove(e) {
      ...
      if (this.centerStart.x + x < 0) {
        ...
        this.wrapStyle.width - this.viewLength
      ) {
      ...
      if (this.centerStart.y + y < 0) {
        ...
        this.wrapStyle.height - this.viewLength
      ) {
    ...
    wLineMousemove(e) {
      ...
      if (change < 0 && (maxLength === this.viewLength || !this.centerLeft))
        return;
      this.startWX = e.clientX;
      let length = this.viewLength - change;
      if (length > maxLength) {
    ...
    nLineMousemove(e) {
      ...
      if (change < 0 && (maxLength === this.viewLength || !this.centerTop))
        return;
      this.startNY = e.clientY;
      let length = this.viewLength - change;
      if (length > maxLength) {
